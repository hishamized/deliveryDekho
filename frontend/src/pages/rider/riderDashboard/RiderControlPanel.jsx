...
import CountdownTimer from "../utilityComponents/CountdownTimer";
/*
    ...
    const fetchData = async () => {
      try {
        ...
        if (sessionRes.status === 200 && sessionRes.data.rider) {
          ...
          setRiderId(id);
          setRider(sessionRes.data.rider);
  ...
  const handlePickupOtpSubmit = async (id, u_id) => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.success) {
        ...
        if (assignmentsRes.status === 200 && assignmentsRes.data.success) {
          setAssignments(assignmentsRes.data.assignments); // this will trigger a re-render
        }
    ...
    <div>
      <div className="flex flex-row p-2 rounded-2xl justify-between items-center  bg-gradient-to-r from-[#4b8378] to-[#2e6158] text-white pt-6 pb-6">
        ...
        <div className="flex flex-col items-start">
          ...
          <p className="text-white-600">Rider - {Rider.name}</p>
        </div>
        <div>
          <button
...
export default RiderControlPanel;
